        ...
        >
          <div class="r-card">
            <div class="thumb-wrap">
              <img :src="thumbSrc(r)" alt="餐點圖片" class="thumb" />
            </div>
...
.tab:hover  { border-color: #2e7d32; }
.tab.active { background: #e6f4ec; border-color: #2e7d32; color: #0b0b0b; }

.slider { position: relative; overflow: hidden; --top-gap: 24px; }
.track  { display: flex; transition: transform .35s ease; padding-top: var(--top-gap); }
.slide  { padding: 0 8px; box-sizing: border-box; }

/* 卡片 */
.r-card     { display: flex; flex-direction: column; align-items: center; text-align: center; }
.thumb-wrap { width: 170px; height: 170px; border-radius: 999px; background: #fff; display: grid; place-items: center; box-shadow: 0 8px 16px rgba(0,0,0,.08); }
.thumb      { width: 156px; height: 156px; border-radius: 999px; object-fit: cover; display: block; }

.r-name { font-size: 22px; font-weight: 900; color: #0f172a; margin-top: 10px; }
.r-sub  { color: #111; font-size: 18px; line-height: 1.35; }
.r-sub-1 { margin-top: 8px; }
.r-sub-2 { margin-top: 2px; }
...
.nav {
  ...
  background: #fff; box-shadow: 0 2px 8px rgba(0,0,0,.15); cursor: pointer; opacity: .95;
}
...
.dot  { width: 8px; height: 8px; border-radius: 999px; background: #d1d5db; border: none; cursor: pointer; }
.dot.active { background: #1a7f3c; transform: scale(1.1); }

/* 手機 */
@media (max-width:768px){
  .thumb-wrap { width: 180px; height: 180px; }
